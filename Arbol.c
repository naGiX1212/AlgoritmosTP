...
unsigned leerDesdeArchivoBin(void ** d, void * pf, unsigned tam, unsigned pos, void * params)
{
    fseek((FILE*)pf, pos * tam, SEEK_SET);
    return fread(*d, tam, 1, (FILE*)pf) != EOF;
}

int cargarDesdeDatosOrdenadosRec(tArbolBinBusq * p, void * ds, unsigned tam, unsigned (*leer)(void **, void *, unsigned, unsigned, void *params),
                                int li, int ls, void * params)
{
    ...
    if((r = cargarDesdeDatosOrdenadosRec(&(*p)->izq, ds, tam, leer, li, m-1, params)) != TODO_BIEN)
        return r;
    return cargarDesdeDatosOrdenadosRec(&(*p)->der, ds, tam, leer, m+1, ls, params);
}
...
int cargarArchivoBinOrdenadoAbiertoArbolBinBusq(tArbolBinBusq * p, FILE * pf, unsigned tamInfo)
{
    ...
    return cargarDesdeDatosOrdenadosRec(p, pf, tamInfo, leerDesdeArchivoBin, 0, cantReg-1, NULL);
}
...
int cargarArchivoBinOrdenadoArbolBinBusq(tArbolBinBusq * p, const char * path, unsigned tamInfo)
{
    ...
    fseek(pf, 0L, SEEK_END);
    cantReg = ftell(pf)/tamInfo;
    r = cargarDesdeDatosOrdenadosRec(p, pf, tamInfo, leerDesdeArchivoBin, 0, cantReg-1, NULL);
    fclose(pf);
    return r;
}
                                            ...
                                            unsigned (*leer)(void **, void *, unsigned, unsigned, void *params), void * params)
{
    if(*p || !ds)
    return 0;
    return cargarDesdeDatosOrdenadosRec(p, ds, tam, leer, 0, cantReg-1, params);
}
